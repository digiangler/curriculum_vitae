# 職務経歴書

最終更新日： 2025/05/23

## 🐾 基本情報

### 📍 名前

- 非公開（仕事場や交流の場で是非お会いしましょう！名刺交換しましょう！）

### 📍 ハンドルネーム

- **Digi-Angler（デジアングラー）**

### 📍 ペンネーム

- **おおとろ**

### 📍 肩書き

- **フリーランス開発者（フルサイクル | 個人開発者）**
- **プロンプトエンジニア**
- **ウェブデザイナー（UX/UI）**
- **ライター（内省的ストーリーテラー）**

### 📍 前職業

- **システムエンジニア兼プログラマー**
- **ウェブデザイナー**
- **セールスライター**

### 🎯 ビジョン

- **自分のサービスを世に出し、持続的な収益を得ること**
- **仲間と一緒に大きな挑戦をすること**
- **家族に家を建ててあげること**

### 📱 SNS

- [X](https://x.com/digiangler) - [Bluesky](https://bsky.app/profile/digiangler.bsky.social) - [Instagram](https://instagram.com/digi_angler) - [Threads](https://www.threads.net/@digi_angler) - [note](https://note.com/digiangler777) - [Qiita](https://qiita.com/digiangler) - [Zenn](https://zenn.dev/digiangler) - [Dribbble](https://dribbble.com/digi-angler)

<!--
## ⚡ 概要

🇯🇵 北海道育ち。
約8年間、関東にてシステムエンジニアとして働いていました。
小さい頃からモノをつくるのが好きで、学生の頃からプログラミング言語を学ぶ。
卒業後、マークアップエンジニアやシステムエンジニア、プログラマとして、某メーカーさんの組み込みシステム開発や、某メーカーさんの医療システム開発、某企業・著名人のホームページ作成と幅広く業務に携わる。

東日本大震災直後、「**本当にやりたいこと**」を実現するためにキャリアチェンジしようと脱サラを決意。
独立後、セールスライティングのスキルと出会い、フリーランスとしてセールスライターやコンテンツライター、ウェブ系エンジニア、ウェブデザイナーなど、幅広く活動中。
ライターは、ペンネーム”おおとろ”として活動しています。
無類のコーヒー愛好家。

現在は、フリーランス開発者（フルサイクル | 個人開発者）として、さまざまな案件を好きな仲間と開発したり、オリジナルのウェブサイトやアプリをつくって運営すること（サブスク）も考えております。

趣味は、ポートフォリオ作成、アウトドア（釣り、キャンプ）、珈琲、読書、音楽鑑賞。
-->

## 🦄 モノづくりと自由を追い求めた道 —— ぼくのストーリー

これからぼくのストーリーを読んでくださる皆さんは、きっと「自分の道を自分で切り開きたい」と考える方や、「好きなことを仕事にしたい」と夢見る方々なのではないでしょうか。ぼくのこれまでの人生をストーリー仕立てでお届けします。学生時代から現在、そしてこれからのビジョンまで、少し長めの旅路になりますが、珈琲でも飲みながら一緒に歩んでみませんか？

要約すると、学生時代の北海道でのモノづくりへの情熱、サラリーマン時代の安定と違和感の狭間での葛藤、東日本大震災をきっかけとした脱サラ決断、フリーランスとしての苦悩と充実感、現在のフリーランス開発者としての取り組み、そして将来のビジョンまで、具体的なエピソードを交えながら描写しています。

### 学生時代：好奇心が導いたプログラミングの世界

ぼくの物語は、北海道の自然豊かな環境で始まります。小さい頃から「モノをつくる」のが大好きでした。紙とペンで空想の街を描いたり、レゴブロックでオリジナルの宇宙船を作ったり。プラモデルもただ組み立てるだけじゃ満足できなくて、色を塗り替えたりパーツを付け足したりしてましたね。皆さんにも、そんな「何かを作り上げたい！」という衝動を感じた瞬間ってありませんか？

中学生になると、学校の授業ではじめてパソコンに触れる機会がありました。正直、当時は「パソコン＝ゲーム機」くらいの感覚だったんですが（笑）、ある日、HTML や VBA というものを知ったんです。簡単なコードを書くだけで、画面に自分の作ったページが表示される。それが楽しくて仕方なかった。そこから独学で CSS や JavaScript を学び始め、掲示板や小さなホームページを作っては友達に見せていました。コードが動いた瞬間の「やった！」という感覚は、今でも鮮明に覚えています。

高校や専門学校に進む頃には、「将来はモノづくりに関わる仕事がしたい」という思いが芽生えていました。授業が終わると、家に帰ってひたすらコードを書く日々。時にはバグに悩まされて頭を抱えたりもしましたが、試行錯誤の先に「動いた！」という達成感があるから、やめられなかったんです。この頃のぼくは、まさに好奇心の塊。皆さんも、何かに夢中になったあの純粋な気持ちを思い出してみてください。それが、ぼくの原点です。

### サラリーマン時代：安定と違和感の間で揺れて

専門学校を卒業後、ぼくは関東近郊の企業にシステムエンジニアとして就職しました。念願の「プロのエンジニア」としての第一歩です。最初は Delphi や Visual Basic、C# などを使った組み込みシステム開発や、医療システムの構築に携わりました。某メーカーのプロジェクトでは、コードが医療現場で役立つ瞬間を想像してワクワクしたものです。技術を磨きながら、少しずつ成長している実感もありました。

でも、安定した環境に身を置く中で、だんだん違和感を覚えるようになったんです。たとえば、「仕様書通りに作る」という仕事の進め方。確かにスキルは上がるし、学びもある。でも、「自分でゼロから生み出す」機会が少ないことに物足りなさを感じていました。皆さんも、「もっと自由にやりたいのに！」とモヤモヤした経験ってありませんか？ぼくにとって、それはサラリーマン生活の中で日に日に大きくなっていったんです。

さらに、組織の中での働き方に「自分の時間が他人に奪われている」という感覚を持つようになりました。残業や会議で、自分のアイディアを形にする時間が削られる。安定は手に入れたけど、本当にやりたいことはどこにあるんだろう？そんな疑問が頭を離れなくなったんです。この違和感が、後に大きな決断を下すきっかけになるとは、当時は想像もしていませんでした。

### 震災のきっかけで脱サラ：人生を見つめ直す瞬間

そして、人生の転機が訪れます。2011 年 3 月 11 日、東日本大震災。ぼくは関東にいましたが、あの揺れとその後の混乱を目の当たりにし、強烈な衝撃を受けたんです。ニュースで流れる被災地の映像や止まらない余震。日常が一瞬で変わってしまう現実を突きつけられ、「このままでいいのか？」と自問するようになりました。

皆さんも、人生が大きく揺さぶられた瞬間ってありますよね？ぼくの場合、震災が「本当にやりたいことをやるべきだ」という思いを加速させたんです。安定した会社員生活を続けるか、それとも自分の手で未来を切り開くか。悩みながらも、「後悔したくない」という気持ちが勝りました。そして、約 8 年間勤めた会社を辞め、フリーランスとして独立する決断を下したんです。

脱サラ直後は、正直不安だらけでした。貯金を切り崩しながら、まずは Web 制作やシステム開発の案件を引き受けて生活を繋ぐ日々。でも、「自分で選んだ仕事をする」という自由は、サラリーマン時代には味わえなかった充実感をくれました。お金のためだけじゃなく、「自分が作りたいものを作る」楽しさをはじめて実感した瞬間でもあったんです。この選択が正しかったかどうかは、今でもときどき振り返りますが、あの時の決断があったからこそ、今のぼくがいる。そう思えるんです。

### 現在：フリーランス開発者としての挑戦

今、ぼくはフリーランス開発者として活動しています。クライアントの案件をこなしながら、自分のプロダクト開発にも挑戦中。たとえば、フロントエンドからバックエンドまで一貫して手がける Web アプリの開発や、個人で運営するサービスの構築。技術的な幅を広げつつ、「作るだけでなく届ける」ことを意識しています。

実は、独立後にセールスライティングのスキルとも出会いました。これがまたおもしろいんですよ。コードを書くのとはまったく違う視点で、「どうすれば人に伝わるか」「どうすれば行動してもらえるか」を考える。心理学や論理的思考を学びながら、ライターとしての表現力も磨いてきました。今では、ペンネーム「おおとろ」として、こうやって文章を書く仕事も楽しんでいます。

日常は、好きな珈琲を淹れて、静かにコードを書く時間が多いですね。アウトドアが趣味なので、週末には釣りやキャンプに出かけてリフレッシュ。自然の中で焚き火をしながら、「次はどんなサービスを作ろうか」と考えるのが至福の時間です。皆さんも、自分のペースで好きなことを楽しめる時間って、大事だと思いませんか？ぼくにとって、フリーランスは「自分の時間を自分で守る」働き方そのものなんです。

### 今後のビジョン：技術と情熱で未来を描く

さて、ここからは未来の話。ぼくにはいくつか叶えたい夢があります。まずひとつ目は、「**自分のサービスを世に出して、持続的な収益を得ること**」。クライアントワークは楽しいけれど、どうしても「時間を切り売りする」形になりがち。たとえば、サブスクリプション型の Web サービスやアプリを開発・運営して、「仕組みで稼ぐ」モデルを作りたいんです。すでにアイディアを温めているプロジェクトもあるので、近いうちカタチにできたらいいなと思っています。

ふたつ目は、「**仲間と一緒に大きな挑戦をすること**」。フリーランスとして自由に働くのもいいけど、同じ志を持った仲間とチームを組んで、よりスケールの大きなプロダクトを生み出したい。「好きなことを仕事にして、自由に生きる」という価値観を共有できる人たちと一緒に、何かを成し遂げる。そんな未来を想像するとワクワクしますね。

そして、もうひとつ、個人的な夢があります。それは、親孝行として「**家族に家を建ててあげること**」。独立してからずっと支えてくれた両親への感謝を、形にしたいんです。そのためにも、技術を磨き、ビジネスを成長させて、経済的な自由を手に入れる。それが、ぼくのこれからの目標です。

### さいごに：皆さんと一緒に未来を描きたい

ここまで読んでくださった皆さん、ありがとう。学生時代の好奇心、サラリーマン時代の葛藤、震災をきっかけにした脱サラ、そして今と未来のビジョン。ぼくの人生は、「モノづくりが好き」「自由に生きたい」というシンプルな想いから始まり、今もその軸は変わっていません。

このストーリーを通して、皆さんの心に何か響くものがあれば嬉しいです。「自分も何か始めたい」「自由に働きたい」と思っているなら、ぜひ一歩踏み出してみてください。最初は小さな一歩でもいいんです。ぼくだって、震災後の不安な一歩から始まりました。それが積み重なって、今の自分を作っているんですから。

これからも、フリーランス開発者として、ライターとして、皆さんに役立つものをお届けしていきたい。そして、いつかこの GitHub で「新しいサービスをリリースしました！」と報告できる日が来ることを楽しみにしています。それではまた、どこかでお会いすることを願って。

## 🔥 デジアングラーの由来

デジタル（digital）とアングラー（angler）の造語です。

アングラーとは、ギリシャ語の「曲がった」という言葉「アングル」の由来で、釣り針のこともアングルと言い、そこからアングルをする人なのでアングラー（魚を釣る人、釣り師）と呼ばれるようです。

また、釣り師と呼ばれるアイドルがいるのをご存知でしょうか？握手会などで神対応をしてくるメンのことです。

このふたつの釣り師を融合し、『**言葉（フック）でお客さんを集めたい ＝ 言葉の釣り師**』になろうということで、「デジアングラー」と名付けました。

## 🌟 スキル

### エンジニアに必要な最低限のスキル

| 基本的な IT スキル           | 自己評価 |
| :--------------------------- | :------- |
| 論理的思考力                 | ★★★★☆    |
| コミュニケーション能力       | ★★★★☆    |
| ヒアリング・提案能力         | ★★★☆☆    |
| 最新技術に関する情報収集能力 | ★★★★★    |
| 問題解決力                   | ★★★★☆    |
| 資料作成スキル               | ★★★★★    |
| 英語力                       | ★☆☆☆☆    |

### 今までに携わってきた技術

### 〜 開発言語 〜

#### 💥 マークアップ

| 言語                  | 評価  | 経験年数  |
| :-------------------- | :---: | :-------- |
| **HTML5**             | ★★★★★ | 10 年以上 |
| **XML**               | ★★★★★ | 10 年以上 |
| **XHTML**             | ★★★★★ | 10 年以上 |
| **CSS3（Sass/SCSS）** | ★★★★★ | 10 年以上 |
| **Markdown**          | ★★★★★ | 10 年以上 |

#### 💥 フロントエンド

| 言語           | 評価  | 経験年数  |
| :------------- | :---: | :-------- |
| **JavaScript** | ★★★★★ | 10 年以上 |
| **TypeScript** | ★★★★★ | 8 年以上  |

#### 💥 バックエンド

| 言語              | 評価  | 経験年数  |
| :---------------- | :---: | :-------- |
| **SQL**           | ★★★★★ | 10 年以上 |
| **PHP**           | ★★★★★ | 10 年以上 |
| **Ruby**          | ★★★★☆ | 8 年以上  |
| **Python**        | ★★★★☆ | 7 年以上  |
| **C Sharp（C#）** | ★★★☆☆ | 1 年以上  |

#### 💥 レガシー

| 言語             | 評価  | 経験年数  |
| :--------------- | :---: | :-------- |
| **SGML**         | ★★★★★ | 10 年以上 |
| **CGI**          | ★★★★★ | 3 年以上  |
| **COBOL**        | ★★★☆☆ | 1 年以上  |
| **Visual Basic** | ★★★★★ | 10 年以上 |
| **VBA**          | ★★★★★ | 10 年以上 |
| **Delphi**       | ★★★★☆ | 2 年以上  |
| **Perl**         | ★★★★☆ | 1 年以上  |

#### 💥 その他

| 言語         | 評価  | 経験年数  |
| :----------- | :---: | :-------- |
| **正規表現** | ★★★★★ | 10 年以上 |

#### 💥 勉強中

| 言語                  | 評価  | 経験年数   |
| :-------------------- | :---: | :--------- |
| **C 言語**            | ★★☆☆☆ | 3 か月以上 |
| **C++**               | ★★☆☆☆ | 3 か月以上 |
| **Java**              | ★★★☆☆ | 3 か月以上 |
| **Scala**             | ★★★☆☆ | 1 か月以上 |
| **Golang（Go 言語）** | ★★★☆☆ | 1 か月以上 |
| **Rust**              | ★★☆☆☆ | 1 か月以上 |
| **R**                 | ★★☆☆☆ | 1 か月以上 |
| **Elixir**            | ★★☆☆☆ | 3 か月以上 |
| **Swift**             | ★★☆☆☆ | 1 か月以上 |
| **Kotlin**            | ★★☆☆☆ | 1 か月以上 |
| **Dart**              | ★★☆☆☆ | 1 か月以上 |

--

### 〜 フレームワーク / ライブラリ / etc 〜

| 言語           | 評価  | 経験年数  |
| :------------- | :---: | :-------- |
| **Node.js**    | ★★★★★ | 10 年以上 |
| **Deno**       | ★★★☆☆ | 2 年以上  |
| **Bun**        | ★★★☆☆ | 3 年以上  |
| **Vite**       | ★★★★☆ | 2 年以上  |
| **Vitest**     | ★★★★☆ | 1 年以上  |
| **Playwright** | ★★★★☆ | 1 年以上  |
| **Zod**        | ★★★☆☆ | 1 年以上  |

#### 💥 フロントエンド

| 言語             | 評価  | 経験年数   |
| :--------------- | :---: | :--------- |
| **Express**      | ★★★★★ | 10 年以上  |
| **Astro**        | ★★★☆☆ | 3 年以上   |
| **React**        | ★★★★★ | 10 年以上  |
| **Vue.js**       | ★★★★★ | 10 年以上  |
| **Next.js**      | ★★★★★ | 10 年以上  |
| **Nuxt.js**      | ★★★★★ | 10 年以上  |
| **Vanilla**      | ★★★☆☆ | 3 年以上   |
| **Solid**        | ★★★☆☆ | 3 年以上   |
| **Svelte**       | ★★★★☆ | 3 年以上   |
| **GraphQL**      | ★★★★☆ | 8 年以上   |
| **Gatsby**       | ★★★★☆ | 8 年以上   |
| **Preact**       | ★★★★☆ | 3 年以上   |
| **Tailwind CSS** | ★★★★★ | 3 年以上   |
| **PlainCSS**     | ★★★☆☆ | 3 か月以上 |
| **Chakra UI**    | ★★★★☆ | 3 年以上   |
| **Bootstrap**    | ★★★★★ | 5 年以上   |
| **MUI**          | ★★★☆☆ | 5 年以上   |
| **WordPress**    | ★★★★★ | 10 年以上  |
| **Storybook**    | ★★★★☆ | 2 年以上   |
| **shadcn/ui**    | ★★★★☆ | 1 年以上   |
| **date-fns**     | ★★★☆☆ | 1 か月以上 |
| **Lodash**       | ★★☆☆☆ | 1 か月以上 |
| **Jest**         | ★★★☆☆ | 3 年以上   |

#### 💥 バックエンド

| 言語              | 評価  | 経験年数  |
| :---------------- | :---: | :-------- |
| **CakePHP**       | ★★★☆☆ | 8 年以上  |
| **Laravel**       | ★★★★☆ | 8 年以上  |
| **Ruby On Rails** | ★★★★☆ | 10 年以上 |
| **Django**        | ★★★★☆ | 6 年以上  |
| **Flask**         | ★★★☆☆ | 3 年以上  |
| **Auth.js**       | ★★★★☆ | 5 年以上  |
| **NextAuth.js**   | ★★★★☆ | 3 年以上  |

#### 💥 勉強中

| 言語             | 評価  | 経験年数   |
| :--------------- | :---: | :--------- |
| **Htmx**         | ★★★☆☆ | 1 か月以上 |
| **Hono**         | ★★★☆☆ | 半年以上   |
| **Waku**         | ★★★☆☆ | 半年以上   |
| **Angular**      | ★★★☆☆ | 3 か月以上 |
| **Angular2+**    | ★☆☆☆☆ | 1 か月以上 |
| **NestJS**       | ★★☆☆☆ | 3 か月以上 |
| **Spring**       | ★★☆☆☆ | 3 か月以上 |
| **Phoenix**      | ★★☆☆☆ | 1 か月以上 |
| **Axios**        | ★★★☆☆ | 3 か月以上 |
| **React Native** | ★★★☆☆ | 3 か月以上 |
| **Flutter**      | ★★★☆☆ | 3 か月以上 |
| **Stream**       | ★★★☆☆ | 1 か月以上 |

--

### 〜 データベース 〜

#### 💥 RDBMS

| 言語                     | 評価  | 経験年数  |
| :----------------------- | :---: | :-------- |
| **MySQL**                | ★★★★★ | 10 年以上 |
| **PostgreSQL**           | ★★★★★ | 10 年以上 |
| **Oracle Database**      | ★★★☆☆ | 8 年以上  |
| **Microsoft SQL Server** | ★★★☆☆ | 8 年以上  |
| **SQLite**               | ★★★★★ | 10 年以上 |

#### 💥 NoSQL

| 言語        | 評価  | 経験年数 |
| :---------- | :---: | :------- |
| **MongoDB** | ★★★★☆ | 3 年以上 |
| **Redis**   | ★★★☆☆ | 2 年以上 |

--

### 〜 インフラ 〜

#### 💥 勉強中

| 言語                      | 評価  | 経験年数 |
| :------------------------ | :---: | :------- |
| **AWS**                   | ★★★☆☆ | 2 年以上 |
| **Google Cloud Platform** | ★★★☆☆ | 3 年以上 |
| **Azure**                 | ★★★☆☆ | 1 年以上 |

--

### 〜 デザイン 〜

| 言語                   | 評価  | 経験年数  |
| :--------------------- | :---: | :-------- |
| **Figma**              | ★★★★☆ | 5 年以上  |
| **Canva**              | ★★★★★ | 5 年以上  |
| **Affinity Photo**     | ★★★★☆ | 5 年以上  |
| **Affinity Designer**  | ★★★☆☆ | 3 年以上  |
| **Affinity Publisher** | ★★★☆☆ | 3 年以上  |
| **Adobe Photoshop**    | ★★★★☆ | 10 年以上 |
| **MindNode**           | ★★★★★ | 5 年以上  |
| **Miro**               | ★★★★★ | 5 年以上  |

#### 💥 勉強中

| 言語                                        | 評価  | 経験年数   |
| :------------------------------------------ | :---: | :--------- |
| **Affinity Photo 2 （無料トライアル）**     | ★★★☆☆ | 2 週間以上 |
| **Affinity Designer 2 （無料トライアル）**  | ★★★☆☆ | 2 週間以上 |
| **Affinity Publisher 2 （無料トライアル）** | ★★★☆☆ | 2 週間以上 |

--

### 〜 SaaS / PaaS / IaaS / その他 Tools 〜

| 言語                 | 評価  | 経験年数   |
| :------------------- | :---: | :--------- |
| **Docker**           | ★★★★☆ | 6 年以上   |
| **GitHub**           | ★★★★★ | 10 年以上  |
| **GitHub Actions**   | ★★★★☆ | 3 年以上   |
| **Sourcetree**       | ★★★★★ | 10 年以上  |
| **BitBucket**        | ★★★★★ | 10 年以上  |
| **Vagrant**          | ★★★★☆ | 6 年以上   |
| **GitLab**           | ★★★☆☆ | 2 年以上   |
| **Heroku**           | ★★★☆☆ | 2 年以上   |
| **CircleCI**         | ★★★☆☆ | 3 年以上   |
| **Prisma**           | ★★★★☆ | 3 年以上   |
| **Drizzle ORM**      | ★★★★☆ | 1 年以上   |
| **Vercel**           | ★★★★☆ | 3 年以上   |
| **Netlify**          | ★★★★☆ | 3 年以上   |
| **Firebase**         | ★★★★☆ | 8 年以上   |
| **Supabase**         | ★★★★☆ | 1 年以上   |
| **Expo**             | ★★★★☆ | 1 年以上   |
| **Auth0**            | ★★★★☆ | 1 年以上   |
| **Homebrew**         | ★★★★★ | 10 年以上  |
| **Git**              | ★★★★★ | 10 年以上  |
| **pnpm**             | ★★★★☆ | 1 年以上   |
| **npm**              | ★★★★★ | 10 年以上  |
| **yarn**             | ★★★★★ | 10 年以上  |
| **asdf**             | ★★★★★ | 6 年以上   |
| **anyenv**           | ★★★★★ | 5 年以上   |
| **pyenv**            | ★★★★★ | 7 年以上   |
| **Volta**            | ★★★★☆ | 3 年以上   |
| **Gem**              | ★★★★☆ | 6 年以上   |
| **Prettier**         | ★★★★★ | 10 年以上  |
| **ESLint**           | ★★★★★ | 10 年以上  |
| **Biome**            | ★★★☆☆ | 3 か月以上 |
| **Husky**            | ★★★★☆ | 1 年以上   |
| **Anaconda**         | ★★★★☆ | 5 年以上   |
| **Jupyter Notebook** | ★★★★☆ | 4 年以上   |
| **JupyterLab**       | ★★★★☆ | 6 年以上   |
| **Google Colab**     | ★★★★☆ | 3 年以上   |

#### 💥 勉強中

| 言語           | 評価  | 経験年数 |
| :------------- | :---: | :------- |
| **Terraform**  | ★★★☆☆ | 3 年以上 |
| **Cloudflare** | ★★★☆☆ | 3 年以上 |
| **Kubernetes** | ★★★☆☆ | 2 年以上 |
| **Datadog**    | ★★★☆☆ | 1 年以上 |
| **Stripe**     | ★★★☆☆ | 1 年以上 |

#### 💥 OS

| 言語        | 評価  | 経験年数  |
| :---------- | :---: | :-------- |
| **macOS**   | ★★★★★ | 10 年以上 |
| **Windows** | ★★★★★ | 10 年以上 |
| **Linux**   | ★★★★☆ | 10 年以上 |
| **Ubuntu**  | ★★★★☆ | 10 年以上 |

#### 💥 Server

| 言語       | 評価  | 経験年数  |
| :--------- | :---: | :-------- |
| **Nginx**  | ★★★★☆ | 10 年以上 |
| **Apache** | ★★★★☆ | 10 年以上 |

#### 💥 API

| 言語          | 評価  | 経験年数  |
| :------------ | :---: | :-------- |
| **Postman**   | ★★★★★ | 10 年以上 |
| **Rapid API** | ★★★★★ | 3 年以上  |

#### 💥 Dev Tools

| 言語                         | 評価  | 経験年数  |
| :--------------------------- | :---: | :-------- |
| **Zsh**                      | ★★★★★ | 10 年以上 |
| **fish**                     | ★★★★★ | 10 年以上 |
| **Warp**                     | ★★★★★ | 3 年以上  |
| **iTerm2**                   | ★★★★★ | 10 年以上 |
| **Hyper**                    | ★★★★★ | 3 年以上  |
| **Xcode**                    | ★★★☆☆ | 3 年以上  |
| **Visual Studio Code**       | ★★★★★ | 10 年以上 |
| **Cursor**                   | ★★★★☆ | 2 年以上  |
| **IntelliJ IDEA （無料版）** | ★★★★☆ | 6 年以上  |
| **NeoVim**                   | ★★★★★ | 10 年以上 |
| **LazyVim**                  | ★★★★★ | 3 年以上  |
| **TablePlus**                | ★★★★★ | 3 年以上  |

#### 💥 Task Tools

| 言語         | 評価  | 経験年数  |
| :----------- | :---: | :-------- |
| **Chatwork** | ★★★★★ | 10 年以上 |
| **Todoist**  | ★★★★★ | 10 年以上 |
| **TickTick** | ★★★★★ | 6 年以上  |
| **Slack**    | ★★★★★ | 10 年以上 |
| **Discord**  | ★★★★★ | 10 年以上 |
| **Jira**     | ★★★★☆ | 5 年以上  |
| **Asana**    | ★★★★☆ | 3 年以上  |
| **Telegram** | ★★★★★ | 3 年以上  |

#### 💥 Writing Tools

| 言語                 | 評価  | 経験年数  |
| :------------------- | :---: | :-------- |
| **Microsoft 365**    | ★★★★★ | 10 年以上 |
| **Google Documents** | ★★★★★ | 10 年以上 |
| **iWork**            | ★★★★★ | 10 年以上 |
| **Sublime Text**     | ★★★★★ | 10 年以上 |
| **Notion**           | ★★★★★ | 5 年以上  |
| **Obsidian**         | ★★★★★ | 3 年以上  |
| **Ulysses**          | ★★★★★ | 10 年以上 |
| **Craft**            | ★★★★★ | 2 年以上  |
| **NotePlan**         | ★★★★★ | 5 年以上  |
| **Dynalist**         | ★★★★★ | 5 年以上  |
| **Evernote**         | ★★★★★ | 10 年以上 |

#### 💥 Marketing Tools

| 言語                 | 評価  | 経験年数 |
| :------------------- | :---: | :------- |
| **Statusbrew**       | ★★★★★ | 5 年以上 |
| **Google Analytics** | ★★★★★ | 6 年以上 |
| **Google AdSense**   | ★★★★★ | 6 年以上 |
| **Google 広告**      | ★★★★★ | 1 年以上 |

#### 💥 Meeting Tools

| 言語      | 評価  | 経験年数  |
| :-------- | :---: | :-------- |
| **zoom**  | ★★★★★ | 6 年以上  |
| **Teams** | ★★★★★ | 6 年以上  |
| **Skype** | ★★★★★ | 10 年以上 |

#### 💥 Browser

| 言語               | 評価  | 経験年数  |
| :----------------- | :---: | :-------- |
| **Google Chrome**  | ★★★★★ | 10 年以上 |
| **Brave Browser**  | ★★★★★ | 10 年以上 |
| **Microsoft Edge** | ★★★★★ | 8 年以上  |
| **Safari**         | ★★★★★ | 10 年以上 |
| **Sidekick**       | ★★★★★ | 2 年以上  |
| **Arc**            | ★★★★★ | 1 年以上  |

#### 💥 Mailer

| 言語                     | 評価  | 経験年数  |
| :----------------------- | :---: | :-------- |
| **Spark Mail**           | ★★★★★ | 8 年以上  |
| **Microsoft Outlook**    | ★★★★★ | 10 年以上 |
| **Mozilla Thunderbird**  | ★★★★★ | 6 年以上  |
| **Becky! Internet Mail** | ★★★★★ | 4 年以上  |
| **Gmail**                | ★★★★★ | 10 年以上 |

#### 💥 GPT

| 言語                            | 評価  | 経験年数 |
| :------------------------------ | :---: | :------- |
| **GitHub Copilot for Business** | ★★★★★ | 3 年以上 |
| **ChatGPT （GPT-4o）**          | ★★★★★ | 2 年以上 |
| **Gemini**                      | ★★★★☆ | 1 年以上 |
| **Claude**                      | ★★★★☆ | 1 年以上 |
| **Copilot**                     | ★★★★☆ | 1 年以上 |
| **Perplexity**                  | ★★★★☆ | 1 年以上 |

#### 💥 Others

| 言語         | 評価  | 経験年数  |
| :----------- | :---: | :-------- |
| **RayCast**  | ★★★★★ | 3 年以上  |
| **Alfred 5** | ★★★★★ | 10 年以上 |
| **ATOK**     | ★★★★★ | 8 年以上  |

## 📝 主な業務経歴

### 某 IT 企業（正社員として 3 社経験）

- システムエンジニア / プログラマ / テスター / ウェブデザイナー

- 2000 年 4 月 - 2011 年 1 月（10 年 10 か月）

- 某メーカーの医療系システム開発業務と某メーカー携帯電話の評価や設計、開発を始め、某メーカーの大規模サイトからベンチャー企業の小規模サイト、某著名人のホームページデザイン・コーディング業務と幅広く業務に携わりました。

### 過去の実績

- 技術系（システム・ネットワーク）

  - 基本設計、詳細設計、プログラミング、データベース、単体 / 結合テスト、保守 / 運用、メンテンナンス

- クリエイティブ系（Web・ゲーム）

  - デザイン / コーディング、画像 / 動画編集、ローカライズ、アンケートの送付（マクロ）、アクセス解析

--

### 社名：某 IT 会社（主に北海道）

- 在職期間：2000 年 4 月～ 2004 年 10 月

- 雇用形態：正社員

- 業種：ソフトウェア・情報処理

- 職務内容：携帯電話の評価および設計、ホームページの作成、管理、医療系システムの開発に従事

#### ［主なプロジェクト内容］

- **某メーカー製　携帯電話評価**

  - 携帯電話のブラウザ、メーラ、Java アプリの評価業務
  - テスト項目書の作成、テスト、不具合時の報告書の作成を担当

- **某メーカー製　放射線情報システム**

  - HOPE/DrABLE-EX のパッケージソフト開発業務
  - 放射線（RIS）/画像参照機能（PACS）/レポート機能の画面の詳細設計、新規作成（カスタマイズ含む）、帳票の作成、テスト、導入支援までを担当

- **自社 HP 作成・管理、某政治家の HP 更新、管理**

  - ホームページの作成、保守、運用、管理業務
  - 自社の HP のアイデアの提案、デザイン、コーディング、画像作成、アップロード、テストまでを担当

- **某メーカー製　介護事業者支援システム**
  - HOPE/WINCARE V2 のパッケージソフト開発業務
  - オプションシステムの処方オプション/預かり金オプションの詳細設計、画面の新規作成、帳票の作成、テストまでを担当

#### ［開発規模］

- プロジェクトメンバー数 / 5 ～ 30 名
- 開発期間 / 3 か月 ～ 3 年 1 か月

#### ［業務内容］

- 詳細設計、プログラミング、単体/ 結合テスト

#### ［開発環境］

- OS / WindowsXP、2000、NT、9x
- DB / Oracle 9i、10g、SQL Server 2000
- 言語 / VB6.0, HTML, XHTML, DHTML, CSS, SQL
- Tool / Microsoft Office, Visual Studio, Acrobat, Crystal Reports, Dreamweaver

---

### 社名：某 IT 会社（関東圏）

- 在職期間：2005 年 6 月～ 2009 年 7 月

- 雇用形態：正社員

- 業種：ソフトウェア・情報処理

- 職務内容：ホームページの作成、管理、医療系システムの開発に従事

#### ［主なプロジェクト内容］

- **某外資メーカーのホームページ（サポートサイト）の新規作成・修正、保守、運用、管理**

  - 某外資メーカーのサポートサイトの作成、保守、運用、管理業務
  - コーディング（約 3000 ページ）、アップロード、ローカライズ、マクロによるアンケートの送付、アクセス解析を担当
  - 2006 年に LISA・ASP 主催の Ten Best International Support Sites award!に受賞

- **某メーカー製　放射線情報システム**
  - iRad-RS のパッケージソフト開発業務
  - 放射線（RIS）の画面の新規作成からカスタマイズ、帳票の作成、テスト、マニュアル・ドキュメント作成、導入支援までを担当

#### ［開発規模］

- プロジェクトメンバー数 / 5 ～ 30 名
- 開発期間 / 1 年 6 か月 ～ 2 年 7 か月

#### ［業務内容］

- プログラミング、単体/ 結合テスト、ローカライズ、アクセス解析、メンテンナンス

#### ［開発環境］

- OS / WindowsXP
- DB / Oracle 9i、10g
- 言語 / VB6.0, HTML, XHTML, DHTML, CSS, XML, SQL, PHP
- Tool / Microsoft Office, Visual Studio, Visio, Photoshop, Acrobat, Crystal Reports, サイボウズ Office8, Eclipse

---

### 社名：某 IT 会社（関東圏）

- 在職期間：2010 年 10 月～ 2011 年 1 月

- 雇用形態：正社員

- 業種：ソフトウェア・情報処理

- 職務内容：防災系システムの開発に従事

#### ［主なプロジェクト内容］

- **某メーカー製　災害警備情報システム**
  - 災害警備情報システムのパッケージソフト開発業務
  - 防災情報システムの画面のカスタマイズ、帳票の作成、テスト、マニュアル・ドキュメント作成までを担当

#### ［開発規模］

- プロジェクトメンバー数 / 2 ～ 5 名
- 開発期間 / 4 か月 ～ 4 か月

#### ［業務内容］

- プログラミング、単体/ 結合テスト、メンテンナンス

#### ［開発環境］

- OS / WindowsXP、Windows Server2007
- DB / Oracle 11g
- 言語 / ASP.net, Visual C#, HTML, XHTML, CSS, XML, JavaScript, SQL
- Tool / Microsoft Office, Visual Studio2010

--

### サラリーマンとしての活動

某携帯メーカーの業務に携わり、Java アプリをはじめとした評価業務、設計部門へのバグ報告の標準化するためにマニュアルを作成して業務改善にも貢献しました。

某医療系メーカーの大規模システムによる開発にも携わり、病院やクリニック向けの電子カルテ、PACS、オーダリングシステム等のシステム導入、開発を納期を短縮して納品した経験があります。

某外資メーカーの大規模サイトから前所属会社の小規模サイトまで携わり、チームワークを重視した運営・管理能力については自信があります。
また、アクセス解析やアンケート集計により、ユーザーがよりストレスなく利用できるサイトを提供し、顧客満足度を上げて受賞に貢献。

コーディングは、学生のころから独学の癖が付いてますので、学べる環境があればすぐに吸収（スキルアップ）し業務に貢献できます。
Dreamweaver などの有料エディターやフリーエディターによる手打ちの経験があり、新規サイトはもちろん既存サイトのソースコードを読み、そのシステムと連携させるためのウェブデザインとコーディングもできます。
更新作業も多いときは、月平均 300 ページのコンテンツをアップロードしました。
Photoshop、Illustrator の使用経験は、業務・プライベートの両方で画像の編集、アイコンなどの作成をしているため基本操作はたぶん大丈夫です（現在は一切使用していませんので微妙かも）。

--

### フリーランス（主にリモートワーク）

- フリーランス開発者 / UI/UX デザイナー / ライター

- 2011 年 1 月 - 現在

- クライアントの案件をこなしながら、自分のプロダクト開発にも挑戦中。たとえば、フロントエンドからバックエンドまで一貫して手がける Web アプリの開発や、個人で運営するサービスの構築。技術的な幅を広げつつ、「作るだけでなく届ける」ことを意識しています。

### 過去の実績

- システム・ネットワーク系

  - 要求・要件定義
  - 基本設計
  - 詳細設計
  - プログラミング
  - データベース
  - 単体 / 結合テスト
  - 保守 / 運用
  - メンテンナンス

- デザイン系

  - ホームページデザイン
  - ロゴ作成
  - バナー作成
  - 画像編集

- コピー・ライティング系

  - メールマガジン
  - オウンドメディア
  - ブログ・SNS（記事作成）
  - LP の作成

- マーケティング系
  - リサーチ
  - AB テスト
  - 広告代理業
  - 物販の仕入れ・販売

<!-- ### デジアングラーとしての活動

2011年に脱サラして、2019にシステムエンジニアの血が騒ぎ出し、ふたたびカムバック。
現在は最新のプログラミング言語や技術を学びながら、案件やポートフォリオを作成しソフトウェアエンジニア兼ウェブデザイナーとライターのフルサイクルを楽しんでいます。
最終的には、**サブスクリプション型ウェブサービス（アプリ）を作成する**のが第一目標です。

### おおとろとしての活動

書きながらスキルアップしている、ノンジャンルライターです。
一応、セールスライターなので、リサーチ力には自信があります。
執筆経験のないジャンルでも徹底的にリサーチを行い、読者へ有益な情報をわかりやすい言葉で提供致します。
言葉こそすべてを動かします。
言葉は思考の根源だからです。
自分の言葉を獲得した人は強い。

しかし、獲得するには七転八倒して、胸を搔きむしらなければなりません。
生きるとは言葉を獲得する闘いでもあります。 -->

---

## 🗿 座右の銘

### **継続は力なり**

--

## 📌 好きな言葉 / 伝えたいこと

- お客様（フォロワー）とは、内臓と内臓を擦れ合う関係を築いていければと思っております。

- 結果を出している人はとんでもない圧倒的努力をしているんです。
  だから、いつも苦しくていつも憂鬱なんです。
  グダグダ言う前に圧倒的努力をしたらどうでしょう。
  それでも結果は出ません。
  それを乗り越えて乗り越えて、ようやく結果は出るんです。
  その結果が 100 溜まって貴方は一人前だと言われます。
  1000 溜まって仕事ができると評価されます。
  10000 溜まって貴方は伝説になります。

- 毎日が苦しくて辛くて憂鬱なんです。
  毎日耐えているものを貴方は想像できますか？
  想像することが本当にできたら、貴方の最初の一歩は始まります。
  最初の一歩ぐらい踏み出してください。

- できるか、出来ないか、ではない。
  やるか、やらないかだ。
  遠くを見つめて、今やれることを全部やる。

- 不可能を可能にしなければたった 1 回の人生、つまらない。

---

## ダウンロード

### 職務経歴書

📍 [工事中](https://github.com/digiangler/)
